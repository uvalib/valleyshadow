...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:template match="/">
		...
		<xsl:variable name="searchName">Letters</xsl:variable>
		<html>
		<head>
		...
		<xsl:choose>
					...
					<xsl:choose>
						<xsl:when test="$numFound &gt; $rows">
							<div id="resultSummary" style="padding-bottom:1em;"><xsl:value-of select="$numFound"/> Results Found |
								Showing 1 - <xsl:value-of select="$rows"/> | <a href="letters_search">New Search</a></div>
						</xsl:when>
						<xsl:otherwise>
							<div id="resultSummary" style="padding-bottom:1em;"><xsl:value-of select="$numFound"/> Results Found |
								Showing 1 - <xsl:value-of select="$numFound"/> | <a href="letters_search">New Search</a></div>
						</xsl:otherwise>
					</xsl:choose>
			...
			<xsl:otherwise>
				...
				<p>
					<a href="letters_search">Go back to search page</a>
				</p>
			</xsl:otherwise>
		</xsl:choose>
</body>
	...
	<xsl:template name="paging">
		...
		<xsl:if test="$current= '1'">
			<xsl:choose>
				<xsl:when test="not($this_page = '1')">
					<a class="pagingBtn"
						href="letters_search_results.html?q={$q}&amp;rows={$rows}&amp;start={(($this_page - 3) * $rows) + $rows}">Previous</a>
					<xsl:text> | </xsl:text>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="not($this_page = $current)">
				<a class="pagingBtn"
					href="letters_search_results.html?q={$q}&amp;rows={$rows}&amp;start={$this_start}">
		...
		<xsl:if test="$current = $pages">
			<xsl:choose>
				...
				<xsl:otherwise>
					<a class="pagingBtn"
						href="letters_search_results.html?q={$q}&amp;rows={$rows}&amp;start={(($this_page - 1) * $rows) + $rows}">Next</a>
				</xsl:otherwise>
			</xsl:choose>
		...
		<span class="previousPage">
			<xsl:choose>
				<xsl:when test="$numFound &gt; $rows and $start &gt; $previous">
					<a class="pagingBtn"
						href="letters_search_results?q={$q}&amp;rows={$rows}&amp;start={$previous}&amp;hl=true&amp;hl.fl=fulltext&amp;hl.fragsize=200&amp;fl=*,score"
						>« Previous </a>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>« Previous </xsl:text>
		...
		<span class="nextPage">
			<xsl:choose>
				<xsl:when test="$numFound &gt; $rows and $next &lt; $numFound">
					...
					<a class="pagingBtn"
						href="letters_search_results?q={$q}&amp;rows={$rows}&amp;start={$next}&amp;hl=true&amp;hl.fl=fulltext&amp;hl.fragsize=200&amp;fl=*,score"
						> Next »</a>
	...
	<xsl:template name="jump_to_pages">
		...
		<xsl:choose>
			<xsl:when test="not($this_page = $current)">
				<a class="pagingBtn"
					href="letters_search_results?q={$q}&amp;start={$this_start}&amp;rows={$rows}&amp;hl=true&amp;hl.fl=fulltext&amp;hl.fragsize=200&amp;fl=*,score">
					<xsl:value-of select="$current"/>
				</a>
			</xsl:when>
			<xsl:otherwise>
