...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:variable name="db">
		<xsl:choose>
			<xsl:when test="starts-with($q, 'db:man_aug_60')">
				<xsl:text>man_aug_60</xsl:text>
			</xsl:when>
			<xsl:when test="starts-with($q, 'db:man_aug_70')">
				<xsl:text>man_aug_70</xsl:text>
			</xsl:when>
			<xsl:when test="starts-with($q, 'db:man_fr_60')">
				<xsl:text>man_fr_60</xsl:text>
			</xsl:when>
			<xsl:otherwise><xsl:text>man_fr_70</xsl:text></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
...
<html>
	<head>
		<title>Valley of the Shadow: <xsl:value-of select="$county"/> County - <xsl:value-of select="$year"/> Manufacturing Census -- Search Results</title>
...
<!-- note that 'kinds_annual' only occurs in 1860 census data; queries to 1870 will return no results for this field ! -->
	<xsl:template match="doc">
		...
		<tr>
			...
			<td>
				<xsl:value-of select="str[@name='kinds_annual']"/>
			</td>
			<td>
	...
	<xsl:template name="paging">
		...
		<xsl:if test="$current= '1'">
			<xsl:choose>
				<xsl:when test="not($this_page = '1')">
					<a class="pagingBtn"
						href="manu_census_search_results.html?q={$q}&amp;start={(($this_page - 3) * 50) + 50}&amp;rows={$rows}&amp;county={$county}&amp;sort=last asc, first asc"
						>Previous</a>
					<xsl:text> | </xsl:text>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="not($this_page = $current)">
				<a class="pagingBtn"
					href="manu_census_search_results.html?q={$q}&amp;start={$this_start}&amp;rows={$rows}&amp;county={$county}&amp;sort=last asc, first asc">
		...
		<xsl:if test="$current = $pages">
			<xsl:choose>
				...
				<xsl:otherwise>
					<a class="pagingBtn"
						href="manu_census_search_results.html?q={$q}&amp;start={(($this_page - 1) * 50) + 50}&amp;rows={$rows}&amp;county={$county}&amp;sort=last asc, first asc"
						>Next</a>
				</xsl:otherwise>
			</xsl:choose>
